...
class ReplaceInFiles {
  ...
  static* handlerAction(
    ...
    if (shouldSkipBinaryFiles && isBinary(data)) {
      return;
    }

    if (onlyFindPathsWithoutReplace) {
      ReplaceInFiles.extractMatches({
        data,
        encoding,
        from,
        path,
  ...
  static async extractMatches({
    ...
    if(extractPath){
      ...
      let writePath = systemPath.resolve(ePath)

      let fullPath = writePath+'/'+fileName
      let preData = (await systemFs.readFileSync(fullPath)).toString()
      try {
      ...
      preData.push(...updated)
      await fs.writeFile(fullPath, JSON.stringify(preData),{
        encoding,
        flag:'w'
      });
    }
