...
const { CHANGE_FILE,SAVE_MATCHES } = require('./constants');
...
class Patcher {
  ...
  static match({ from, data }) {
    return data.match(from)
  }
  ...
  extract(){
    let result = Patcher.match(this.options);
    helpers.eventEmitter.emit(SAVE_MATCHES, result);
    return result
  }
